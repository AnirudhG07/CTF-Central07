...
This is a nice problems which involves usage of Sleuthkit. Let's see the files we have.
...
```bash
$ icat suspicious.dd.sda1 12 # 12 is number give with fls.
Nothing to see here! But you may want to look here -->
```
Now we need to check where the flag is hidden in the disk image. We can use `strings` to search for the flag:
```bash
...
```
Now we see the location near 0x200400 in the hexdump-
```bash
...
```
We can see the flag formatted in the hexdump. We can extract it using `od`:
